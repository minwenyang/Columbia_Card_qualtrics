...
var repo_site = "https://minwenyang.github.io/columbia-card-task-hot/"
	...
	if (board_type == 2) {
		...
		for (i = 1; i < 33; i++) {
			board += "<div class = square><input type='image' id = " + i +
				" class = 'card_image' src='" +repo_site + "images/beforeChosen.png' onclick = instructCard(this.id)></div>"
		}
		...
		for (i = 1; i < 33; i++) {
			board += "<div class = square><input type='image' id = " + i +
				" class = 'card_image select-button' src='" +repo_site + "images/beforeChosen.png' onclick = chooseCard(this.id)></div>"
		}
	}
	board += "</div>"
	return board
...
var getRound = function() {
	...
	if (roundOver === 0) { //this is for the start of a round
		...
		gameState = appendTextAfter(gameState, 'Gamble Round: ', whichRound)
		...
		for (i = 0; i < clickedGainCards.length; i++) {
			gameState = appendTextAfter2(gameState, "id = " + "" + clickedGainCards[i] + ""," class = 'card_image' src='" +repo_site + "images/chosen.png'", " class = 'card_image select-button' src='" +repo_site + "images/beforeChosen.png' onclick = chooseCard(this.id)")
		}
		return gameState
		...
		for (var i = 1; i < cardArray.length + 1; i++) {
			if (clickedGainCards.indexOf(i) != -1 ) {
				gameState = appendTextAfter2(gameState, "id = " + "" + i + ""," class = 'card_image' src='" +repo_site + "images/chosen.png'", " class = 'card_image select-button' src='" +repo_site + "images/beforeChosen.png' onclick = chooseCard(this.id)")
			} else if (clickedLossCards.indexOf(i) != -1 ) {
				gameState = appendTextAfter2(gameState, "id = " + "" + i + ""," class = 'card_image' src='" +repo_site + "images/loss.png'", " class = 'card_image select-button' src='" +repo_site + "images/beforeChosen.png' onclick = chooseCard(this.id)")
			} else {
				gameState = appendTextAfter2(gameState, "id = " + "" + i + ""," class = 'card_image' src='" +repo_site + "images/beforeChosen.png'", " class = 'card_image select-button' src='" +repo_site + "images/beforeChosen.png' onclick = chooseCard(this.id)")
			}
		}
...
var instructFunction2 = function() {
	...
	var instructTurnCards = function() {
		document.getElementById("8").src = repo_site + 'images/loss.png';
		document.getElementById("2").src = repo_site + 'images/loss.png';
...
var roundCardArray = [] // create a new array to store the number of cards that Ps clicked
var whichGainCards = []
var whichLossCards = []
var prize1 = 0
var prize2 = 0
...
var gameSetup =
	"<div class = cct-box>"+
	"<div class = titleBigBox>   <div class = titleboxLeft><div class = center-text id = game_round>Gamble Round: </div></div>   <div class = titleboxLeft1><div class = center-text id = loss_amount>Loss Amount: </div></div>    <div class = titleboxMiddle1><div class = center-text id = gain_amount>Gain Amount: </div></div>    <div class = titlebox><div class = center-text>How many cards do you want to take? </div></div>     <div class = titleboxRight1><div class = center-text id = num_loss_cards>Number of Loss Cards: </div></div>   <div class = titleboxRight><div class = center-text id = current_round>Current Round Points: 0</div></div>"+
...
var practiceSetup =
	"<div class = practiceText><div class = block-text2 id = instruct1><strong>Practice 1: </strong> As you click on cards, you can see your Round Total change in the box in the upper right.  If you turn over a few cards and then want to stop and go to the next round, click the <strong>Stop/Turn Over</strong> button and then <strong>Next Round</strong>.  If turning over cards seems too risky, you can click the <strong>No Card</strong> button, in which case your score for the round will automatically be zero.  This is a practice round, that looks just like the gamble you will play.  Please select the number of cards you would turn over, given the number of loss cards and the amounts of the gain and loss cards shown below.</div></div>"+
	"<div class = cct-box2>"+
	"<div class = titleBigBox>   <div class = titleboxLeft><div class = center-text id = game_round>Gamble Round: 1</div></div>   <div class = titleboxLeft1><div class = center-text id = loss_amount>Loss Amount: 250</div></div>    <div class = titleboxMiddle1><div class = center-text id = gain_amount>Gain Amount: 30</div></div>    <div class = titlebox><div class = center-text>How many cards do you want to take? </div></div>     <div class = titleboxRight1><div class = center-text id = num_loss_cards>Number of Loss Cards: 1</div></div>   <div class = titleboxRight><div class = center-text id = current_round>Current Round Points: 0</div></div>"+
...
var instructions_block = {
  ...
  pages: [
    ...
    "<p><input type='image' id = '133' src='" +repo_site + "images/beforeChosen.png' onclick = instructButton(this.id)>"+
	'</p></div>',
	...
	"<p><input type='image' src='" +repo_site + "images/chosen.png'>"+
	'<p><strong>The Loss Card:</strong></p>'+
	"<p><input type='image' src='" +repo_site + "images/loss.png'></p>"+
	...
	"<div class = cct-box2>"+
	"<div class = titleBigBox>   <div class = titleboxLeft><div class = center-text>Gamble Round: 1</div></div>   <div class = titleboxLeft1><div class = center-text>Loss Amount: 750</div></div>    <div class = titleboxMiddle1><div class = center-text>Gain Amount: 10</div></div>    <div class = titlebox><div class = center-text>How many cards do you want to take? </div></div>     <div class = titleboxRight1><div class = center-text>Number of Loss Cards: 1</div></div>   <div class = titleboxRight><div class = center-text id = current_round>Current Round Points: 0</div></div>"+
	...
	"<div class = cct-box2>"+
	"<div class = titleBigBox>   <div class = titleboxLeft><div class = center-text>Gamble Round: 1</div></div>   <div class = titleboxLeft1><div class = center-text>Loss Amount: 250</div></div>    <div class = titleboxMiddle1><div class = center-text>Gain Amount: 30</div></div>    <div class = titlebox><div class = center-text>How many cards do you want to take? </div></div>     <div class = titleboxRight1><div class = center-text>Number of Loss Cards: 3</div></div>   <div class = titleboxRight><div class = center-text id = current_round>Current Round Points: 0</div></div>"+
	"<div class = buttonbox><button type='button' class = 'CCT-btn select-button' id = NoCardButton disabled>No Card</button><button type='button' class = 'CCT-btn select-button' class = 'CCT-btn select-button' id = turnButton disabled>STOP/Turn Over</button><button type='button' class = 'CCT-btn select-button' id = collectButton  disabled>Next Round</button></div>"+
...
var continue_instruct_text =
	"Press <strong>enter</strong> to play 5 rounds of formal gamble."
...
var test_node = {
	...
	loop_function: function(data) {
		if (currID == 'collectButton') {
			...
			cardClick = whichClickInRound - 1 // recalculate the number of card cliked in each round
			roundCardArray.push(cardClick)
			roundOver = 0
			roundPoints = 0
			whichClickInRound = 0
			cardClick = 0
